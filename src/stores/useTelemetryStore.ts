...
import type { ActivityEvent, ChartKind, StreamStatus, TelemetryPoint, TimeRange } from '../types';

const stream = new TelemetryStream();

export const useTelemetryStore = defineStore('telemetry', {
  state: () => ({
    points: [] as TelemetryPoint[],
    events: [] as ActivityEvent[],
    status: 'connecting' as StreamStatus,
    paused: false,
    error: '',
    malformedCount: 0,
    timeRange: 300 as TimeRange,
    chartKind: 'line' as ChartKind,
    query: '',
    severity: 'all',
    selectedDatasets: ['throughput', 'latency', 'cpu'],
    theme: (localStorage.getItem('livepulse-theme') || 'dark') as 'dark' | 'light',
    cleanupPayload: null as (() => void) | null,
    cleanupStatus: null as (() => void) | null
  }),
  getters: {
    datasets: () => DATASETS,
    visiblePoints(state) {
  ...
  actions: {
    ingest(payload: unknown) {
      ...
      if (payload.event) {
        ...
        if (this.events.length > MAX_EVENTS) this.events.splice(MAX_EVENTS);
      }

      this.error = '';
    },
    connect() {
      if (this.cleanupPayload || this.cleanupStatus) return;
    ...
    setChartKind(kind: ChartKind) {
      this.chartKind = kind;
    },
    toggleDataset(id: string) {
      if (this.selectedDatasets.includes(id)) {
        if (this.selectedDatasets.length > 1) this.selectedDatasets = this.selectedDatasets.filter((item) => item !== id);
        return;
      }
